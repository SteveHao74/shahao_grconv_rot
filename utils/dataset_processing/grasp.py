...
class GraspRectangles:
    ...
    def load_from_cornell_file(cls, fname):
        ...
        with open(fname) as f:
            while True:
                # Load 4 lines at a time, cor ners of bounding box.
                p0 = f.readline()
                if not p0:
                    break  # EOF
        ...
        grs_return=[]
        gr_sample = random.sample(grs, 1)[0]
        for gr in grs:
            # if (gr.angle - gr_sample.angle) ** 2 < 0.030461741978670857:
            if abs(gr.angle - gr_sample.angle) < math.pi/36:
                grs_return.append(gr)
        return cls(grs_return)

    @classmethod
    def load_from_cornell_file_val(cls, fname):
        """
        Load grasp rectangles from a Cornell dataset grasp file.
        :param fname: Path to text file.
        :return: GraspRectangles()
    ...
    @classmethod
    def load_from_jacquard_file_val(cls, fname, scale=1.0):
        """
        Load grasp rectangles from a Jacquard dataset file.
        :param fname: Path to file.
        :param scale: Scale to apply (e.g. if resizing images)
    ...
    def draw_msra_gaussian(self,heatmap, center, sigma, rr, cc,quality=1):
        mask = np.zeros(heatmap.shape)
        mask[rr,cc] = quality
        tmp_size = sigma * 3
        mu_x = int(center[1] + 0.5)
        mu_y = int(center[0] + 0.5)
        ...
        y = x[:, np.newaxis]
        x0 = y0 = size // 2
        g = np.exp(- ((x - x0) ** 2 + (y - y0) ** 2) / (2 * (sigma+1) ** 2))#jaq:sigma+1
        ...
        #     g[g_y[0]:g_y[1], g_x[0]:g_x[1]])#纯粹高斯分布再处理下交叠处的取最大值
        heatmap[img_y[0]:img_y[1], img_x[0]:img_x[1]] = np.maximum(
            heatmap[img_y[0]:img_y[1], img_x[0]:img_x[1]],
            np.multiply( g[g_y[0]:g_y[1], g_x[0]:g_x[1]], mask[img_y[0]:img_y[1], img_x[0]:img_x[1]]))#先是矩阵对应元素乘法，得到mask效果#处理重叠部分的方式，取最大值
        return heatmap
    ...
    def draw(self, shape, position=True, angle=True, width=True):
        ...
        for gr in self.grs:
            ...
            if position:
                # print("gr.length,gr.width",gr.length,gr.width)
                pos_out=self.draw_msra_gaussian(pos_out,gr.center,gr.length/6,rr,cc,1)#用高斯分布编码抓取质量图
                # pos_out[rr, cc] = 1.0
            if angle:
                # print("angle:{}".format(gr.angle))
                ang_out[rr, cc] = gr.angle
            if width:
                # print("length:{}".format(gr.length))
                width_out[rr, cc] = gr.length

        return pos_out, ang_out, width_out

    def draw_rot(self, shape, position=True, width=True,quality = 1):
        """
        Plot all GraspRectangles as solid rectangles in a numpy array, e.g. as network training data.
        :param shape: output shape
        :param position: If True, Q output will be produced
        ...
        for gr in self.grs:
            ...
            if position:
                pos_out[rr, cc] = quality
                # print("rot_gr.length/6",gr.length/6)
                pos_out=self.draw_msra_gaussian(pos_out,gr.center,gr.length/6,rr,cc,quality)#用高斯分布编码抓取质量图
            if width:
                # print("length:{}".format(gr.length))
                width_out[rr, cc] = gr.length
        return pos_out, width_out
...
# version_0
# def detect_grasps_rot(q_img, width_img=None, no_grasps=1,zoom_factor=1):
#     """
#     Detect grasps in a GG-CNN output.
#     :param q_img: Q image network output
#     :param ang_img: Angle image network output
...
#
#     for i in range(12):
#         l_m = peak_local_max(q_img[i], min_distance=5, threshold_abs=0.0, num_peaks=no_grasps)
#         if len(l_m) :
#             local_max.append(l_m)
#             local_max_q.append(q_img[i][l_m[0][0],l_m[0][1]])
#         else:
#             local_max.append([0,0])
#             local_max_q.append(0)
#
#     index_max = local_max_q.index(max(local_max_q))
#     grasp_angle = -math.pi/2 + index_max* math.pi/12
#
#     try:
#         depth_img = rotate(q_img[index_max], grasp_angle / np.pi * 180, center=None, mode='edge', preserve_range=True).astype(
#             q_img[index_max].dtype)
#     except IndexError:
#         print(index_max)
#         print(local_max_q)
#     depth_img_lm = peak_local_max(depth_img, min_distance=5, threshold_abs=0.0, num_peaks=no_grasps)
#
#     grasps = []
#     for grasp_point_array in depth_img_lm:
#         grasp_point = tuple(grasp_point_array)
#         g = Grasp(grasp_point, grasp_angle)
#         if width_img is not None:
#             # g.length = width_img[index_max][grasp_point]
#             # print(zoom_factor)
#             g.length = 70*zoom_factor.item()
#             g.width = g.length/2
#
#         grasps.append(g)
#
...
def detect_grasps_rot(q_img, width_img=None, no_grasps=1,zoom_factor=1):
    ...
    for i in range(step):
        l_m = peak_local_max(q_img[i], min_distance=5, threshold_abs=0.0, num_peaks=1)
        # print('peak for img:{}'.format(i))
        # qi = np.sum(q_img[i][l_m[0][0]-5:l_m[0][0]+5,l_m[0][1]-5:l_m[0][1]+5])
        
        # print("147",i)

        if len(l_m) :
            qi = q_img[i][l_m[0][0],l_m[0][1]]
            local_max.append(l_m)
            local_max_q.append(qi)
        else:
            local_max.append([0,0])
            local_max_q.append(0)
    import pdb;pdb.set_trace()
    ...
    try:
        depth_img = rotate(q_img[index_max], grasp_angle / np.pi * 180, center=None, mode='edge', preserve_range=True).astype(
            q_img[index_max].dtype)
        print(index_max,local_max_q)
    except IndexError:
        print(index_max)
        print(local_max_q)
    depth_img = gaussian(depth_img, 4.0, preserve_range=True)
    depth_img_lm = peak_local_max(depth_img, min_distance=5, threshold_abs=0.0, num_peaks=no_grasps)
    print("depth_img_lm",depth_img_lm,np.array(depth_img).mean())
    grasps = []
    for grasp_point_array in depth_img_lm:
        grasp_point = tuple(grasp_point_array)
        g = Grasp(grasp_point, grasp_angle)
